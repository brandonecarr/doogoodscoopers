...
export default function NewDripPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      else setError(data.error || "Failed to create drip");
    } catch {
      setError("Failed to create drip");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-24 lg:pb-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-navy-900">New Drip Campaign</h1>
      ...
      >
        ...
        {saving ? "Creating…" : "Create drip"}
      </button>
      <p className="text-xs text-gray-400">
        Once active, new {leadTypes.length ? "" : "matching "}leads are enrolled automatically (within ~1 min) and receive the sequence. Opt-outs and replies stop it.
      </p>
    </div>
  );
}
