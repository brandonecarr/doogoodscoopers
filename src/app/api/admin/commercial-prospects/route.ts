...
import { importProspects } from "@/lib/commercial-prospects";
...
export async function POST(request: Request) {
  ...
  const rows = Array.isArray((body as { rows?: unknown[] })?.rows) ? (body as { rows: unknown[] }).rows : [body];
  ...
  try {
    ...
    if (rows.length === 1 && r.created === 0) return NextResponse.json({ success: false, error: r.skipped[0]?.reason || "Not added" }, { status: 400 });
