  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const map = new mapboxgl.Map({
        ...
        zoom: 16.4, pitch: 58, bearing: -18, attributionControl: false,
      });
      mapRef.current = map;
      map.on("style.load", () => {
      ...
      map.on("error", (e: { error?: { message?: string } }) => {
        ...
        if (!fellBack && /style|standard|not\s*found|sprite|glyph/i.test(msg)) {
          fellBack = true;
          try { map.setStyle("mapbox://styles/mapbox/light-v11"); } catch {}
        }
      });
    ...
    return () => { cancelled = true; ro?.disconnect(); mapRef.current?.remove(); mapRef.current = null; };
