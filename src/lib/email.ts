...
/**
 * Send welcome email to new customer after successful registration
 */
export async function sendWelcomeEmail(data: WelcomeEmailData): Promise<boolean> {
  ...
  const emailText = generateWelcomeEmailText(data);
...
/**
 * Send notification to business owner about new customer signup
 */
export async function sendNewCustomerNotificationEmail(data: WelcomeEmailData): Promise<boolean> {
  const transporter = createTransporter();
  ...
  try {
    await transporter.sendMail({
      from: `"${fromName}" <${fromEmail}>`,
      to: notifyEmail,
      subject: `New Customer Signup: ${data.customerName}`,
      text: `
New Customer Signup!
...
Number of Dogs: ${data.numberOfDogs || "Not specified"}
Service Frequency: ${data.frequency || "Not specified"}
${data.recurringPrice ? `Per Visit Price: $${data.recurringPrice}` : ""}
${data.nextServiceDate ? `First Service Date: ${data.nextServiceDate}` : ""}

---
This customer has completed registration through the ${SITE_CONFIG.name} website.
      `.trim(),
    });

    console.log(`New customer notification email sent to ${notifyEmail}`);
    return true;
  } catch (error) {
        ...
        <table role="presentation" style="max-width: 600px; width: 100%; border-collapse: collapse; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          <tr>
            <td style="background-color: ${primaryColor}; padding: 32px 40px; text-align: center;">
              ...
              <p style="margin: 8px 0 0 0; color: rgba(255, 255, 255, 0.9); font-size: 14px;">
                Your cleaner yard journey starts now
              </p>
            </td>
          </tr>
          <tr>
            <td style="padding: 40px;">
              ...
              <p style="margin: 0 0 24px 0; color: #4b5563; font-size: 16px; line-height: 1.6;">
                Thank you for signing up with ${SITE_CONFIG.name}! We're thrilled to have you as a customer.
              </p>
              <table role="presentation" style="width: 100%; border-collapse: collapse; margin-bottom: 24px;">
                <tr>
                  <td style="background-color: #f0fdfa; border: 2px solid ${accentColor}; border-radius: 8px; padding: 24px;">
                    <h3 style="margin: 0 0 16px 0; color: ${primaryColor}; font-size: 18px;">Your Service Details</h3>
                    <table role="presentation" style="width: 100%; border-collapse: collapse;">
                      ${data.frequency ? `<tr><td style="padding: 6px 0; color: #6b7280;">Service Frequency:</td><td style="padding: 6px 0; color: #1f2937; text-align: right; font-weight: 500;">${data.frequency}</td></tr>` : ""}
                      ${data.numberOfDogs ? `<tr><td style="padding: 6px 0; color: #6b7280;">Number of Dogs:</td><td style="padding: 6px 0; color: #1f2937; text-align: right; font-weight: 500;">${data.numberOfDogs}</td></tr>` : ""}
                      ${data.recurringPrice ? `<tr><td style="padding: 6px 0; color: #6b7280;">Per Visit Price:</td><td style="padding: 6px 0; color: #1f2937; text-align: right; font-weight: 500;">$${data.recurringPrice}</td></tr>` : ""}
                      ${data.address ? `<tr><td style="padding: 6px 0; color: #6b7280;">Service Address:</td><td style="padding: 6px 0; color: #1f2937; text-align: right; font-weight: 500;">${data.address}</td></tr>` : ""}
                    </table>
                  </td>
                </tr>
              </table>
              <h3 style="margin: 0 0 12px 0; color: ${primaryColor}; font-size: 18px;">What to Expect</h3>
              <ol style="margin: 0 0 24px 0; padding-left: 20px; color: #4b5563; font-size: 15px; line-height: 1.8;">
                <li>We'll schedule your first service within the next few business days</li>
                <li>You'll receive a notification when our technician is on the way</li>
                <li>After each visit, you'll get a confirmation that the job is complete</li>
                <li>Sit back, relax, and enjoy your poop-free yard!</li>
              </ol>
              <table role="presentation" style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td align="center" style="padding-top: 8px;">
                    <p style="margin: 0 0 16px 0; color: #6b7280; font-size: 14px;">Have questions? We're here to help!</p>
                    <a href="tel:${SITE_CONFIG.phone.replace(/[^0-9]/g, "")}" style="display: inline-block; background-color: ${accentColor}; color: #ffffff; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-size: 16px; font-weight: 600;">
                      Call Us: ${SITE_CONFIG.phone}
                    </a>
                  </td>
...
Thank you for signing up! We're thrilled to have you as a customer.

YOUR SERVICE DETAILS
${data.frequency ? `- Service Frequency: ${data.frequency}` : ""}
${data.numberOfDogs ? `- Number of Dogs: ${data.numberOfDogs}` : ""}
${data.recurringPrice ? `- Per Visit Price: $${data.recurringPrice}` : ""}
${data.address ? `- Service Address: ${data.address}` : ""}

WHAT TO EXPECT
1. We'll schedule your first service within the next few business days
2. You'll receive a notification when our technician is on the way
3. After each visit, you'll get a confirmation that the job is complete
4. Sit back, relax, and enjoy your poop-free yard!
